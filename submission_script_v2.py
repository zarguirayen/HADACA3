...
def program(mix=None, ref=None, **kwargs):
  ...
  # ──────────────────────────────────────────────
  #  HYPER-PARAMETERS  (tune via leaderboard feedback)
  # ──────────────────────────────────────────────
  EPS         = 1e-6
  CAL_EPS     = 1e-12
  N_RNA       = 1200    # marker genes kept for RNA
  N_MET       = 4000    # CpG sites kept for methylation
  CAL_ALPHA   = 0.85    # blend weight for calibration (0 = ignore calib, 1 = full calib)
  RIDGE_ALPHA = 1e-2    # ridge penalty for calibration model
  NU_SVR_NU   = 0.5     # nu-SVR nu parameter  (try 0.25 / 0.5 / 0.75)
  NU_SVR_C    = 1.0     # nu-SVR regularisation
  USE_ENSEMBLE = True   # blend nu-SVR + NNLS in CLR space
  ...
  def geo_mean_proportions(prop_list, weights=None):
      """Average multiple proportion DataFrames in CLR space (= geometric mean)."""
      if weights is None:
  ...
  def select_features_specificity(ref_df, n_keep):
      """
      Rank genes by: specificity × coefficient-of-variation.
      Specificity = max_expression / mean_expression across cell types.
      This favours marker genes that are highly expressed in ONE type.
      """
      ...
      scores = pd.Series(score, index=ref_df.index)
      return scores.sort_values(ascending=False).head(n_keep).index

  def select_features_fstat(ref_df, n_keep):
      """
      F-statistic style: between-group variance / within-group variance proxy.
      High F = gene is discriminative across cell types.
      """
      if n_keep is None or n_keep >= ref_df.shape[0]:
          return ref_df.index
      R = ref_df.to_numpy(float)
      ...
      # within variance approximated by variance of each column around its own mean
  ...
  def prep_align(mix_df, ref_df, n_keep=None, log1p=True, feature_method='specificity'):
      ...
      # Row-wise mean scaling (reference-based)
  ...
  def nusvr_deconv(mix_df, ref_df):
      """
      CIBERSORT-style nu-SVR deconvolution.
      For each mixture sample:
        - Z-score each gene using reference row statistics
        - Fit nu-SVR: X = ref_scaled (genes × celltypes), y = mix_scaled (genes,)
        - SVR coefficients (clipped to ≥0) are the proportions
      """
      R         = ref_df.to_numpy(dtype=float)
      celltypes = list(ref_df.columns)

      # Per-gene statistics from reference (stable across samples)
      R_mean = R.mean(axis=1, keepdims=True)          # (genes, 1)
      R_std  = R.std(axis=1, keepdims=True) + 1e-8    # (genes, 1)
      R_sc   = (R - R_mean) / R_std                   # (genes, celltypes)

      props, errs = [], []
      for s in mix_df.columns:
          y    = mix_df[s].to_numpy(dtype=float)
          y_sc = (y - R_mean.flatten()) / R_std.flatten()
  ...
  # ──────────────────────────────────────────────
  #  BASE PREDICTION  (RNA + methylation fusion)
  # ──────────────────────────────────────────────
  def base_predict(mix_rna, ref_rna, mix_met, ref_met):
      # ---- RNA deconvolution (ensemble of nu-SVR + NNLS) ----
      ...
      if USE_ENSEMBLE:
          ...
          # Error = weighted mean of both errors (used later for met fusion)
      ...
      # ---- Methylation deconvolution (optional) ----
      ...
      # ---- Fuse RNA + methylation (inverse-error weighting) ----
      ...
      if len(common) > 0:
          ...
          # Fuse in CLR space for compositional correctness
  ...
  # ──────────────────────────────────────────────
  #  CALIBRATION  (linear map in CLR space)
  # ──────────────────────────────────────────────
  def calib_paths():
